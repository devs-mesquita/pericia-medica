<p>Olá {{ substr($requerimento->nome, 0, strpos($requerimento->nome, ' ')) }}, a data de atendimento do seu requerimento precisou ser realocada pelo nosso setor.</p>
<li><b>Motivo da Realocação:</b> {{ $reagendamento->justificativa_realocacao }}</li>
...
<h3 style="margin-bottom: 5px;">Requerimento Realocado</h3>
<ul style="margin-top: 0;">
    <li><b>Protocolo do seu requerimento:</b> {{ $requerimento->protocolo }}</li>
    <li><b>Direcionamento:</b> {{ $realocacao->direcionamento->name }}</li>
    <li><b>Nova data/hora agendada:</b> {{ date('d/m/Y', strtotime($realocacao->agenda_datetime)) }} às {{ date('H:i', strtotime($realocacao->agenda_datetime)) }} horas.</li>
